...
class HalfCheetahBulletEnv(WalkerBaseBulletEnv):
    def __init__(self, render=False):
        ...
        WalkerBaseBulletEnv.__init__(self, self.robot, render)
        print(
            "PyBullet Halfcheetah-v5: reward = progress + (njoint_at_limit * -0.1), terminate also when z < 0.3"
        )
...
class AntBulletEnv(WalkerBaseBulletEnv):
    ...
    def step(self, a):
        ...
        return (
            state,
            progress + 0.01 + stall_cost + joints_at_limit_cost,
            bool(done),
            {"progress": progress},
        )
...
class HumanoidBulletEnv(WalkerBaseBulletEnv):
    def __init__(self, robot=Humanoid(), render=False):
        ...
        WalkerBaseBulletEnv.__init__(self, self.robot, render)
        print(
            "PyBullet Humanoid-v5: reward = progress + 1.0 + (jexcess * -10.0) + (nJLimits * -0.1) + (angleoffset * -0.1): init_range [-0.03,0.03]"
        )
    ...
    def step(self, a):
        ...
        angle_offset_cost = (
            self.robot.angle_to_target * self.robot.angle_to_target
        ) * -0.1
